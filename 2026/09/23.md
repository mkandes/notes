# Docusaurus Evaluation

```
mkandes@hardtack:~$ conda create -n docusaurus
Retrieving notices: done
Channels:
 - conda-forge
Platform: linux-64
Collecting package metadata (repodata.json): done
Solving environment: done

## Package Plan ##

  environment location: /home/mkandes/software/miniforge/26.3.2-1/envs/docusaurus



Proceed ([y]/n)? y


Downloading and Extracting Packages:

Preparing transaction: done
Verifying transaction: done
Executing transaction: done
#
# To activate this environment, use
#
#     $ conda activate docusaurus
#
# To deactivate an active environment, use
#
#     $ conda deactivate

mkandes@hardtack:~$ conda search nodejs
Loading channels: done
# Name                       Version           Build  Channel             
nodejs                       10.12.0   hf484d3e_1000  conda-forge         
nodejs                       10.12.0      hfc679d8_0  conda-forge         
nodejs                       10.15.3      hf484d3e_0  conda-forge         
nodejs                       10.16.3      h10a4023_0  conda-forge         
nodejs                       10.22.0      hf5d1a2b_0  conda-forge         
nodejs                        10.4.1               0  conda-forge         
nodejs                        10.8.0               0  conda-forge         
nodejs                        10.8.0   hf484d3e_1001  conda-forge         
nodejs                        10.8.0      hfc679d8_1  conda-forge         
nodejs                       11.10.0      hf484d3e_0  conda-forge         
nodejs                       11.11.0      hf484d3e_0  conda-forge         
nodejs                       11.14.0      he1b5a44_0  conda-forge         
nodejs                       11.14.0      he1b5a44_1  conda-forge         
nodejs                        11.4.0   hf484d3e_1000  conda-forge         
nodejs                        11.5.0   hf484d3e_1000  conda-forge         
nodejs                        11.6.0   hf484d3e_1000  conda-forge         
nodejs                        11.7.0      hf484d3e_0  conda-forge         
nodejs                        11.8.0      hf484d3e_0  conda-forge         
nodejs                        11.9.0      hf484d3e_0  conda-forge         
nodejs                        12.1.0      he1b5a44_1  conda-forge         
nodejs                       12.10.0      h10a4023_0  conda-forge         
nodejs                       12.11.1      h10a4023_0  conda-forge         
nodejs                       12.12.0      h10a4023_0  conda-forge         
nodejs                       12.13.0      h10a4023_0  conda-forge         
nodejs                       12.14.1      h67efad7_0  conda-forge         
nodejs                       12.16.0      h67efad7_0  conda-forge         
nodejs                       12.16.1      h67efad7_0  conda-forge         
nodejs                       12.18.3      h628520e_1  conda-forge         
nodejs                       12.18.3      h67efad7_0  conda-forge         
nodejs                       12.19.0      h628520e_0  conda-forge         
nodejs                       12.19.0      h8b53aa1_3  conda-forge         
nodejs                       12.19.0      hdbd6b0a_1  conda-forge         
nodejs                       12.19.0      hfa01f41_2  conda-forge         
nodejs                       12.22.6      h8b53aa1_0  conda-forge         
nodejs                        12.3.0      he1b5a44_0  conda-forge         
nodejs                        12.3.1      he1b5a44_0  conda-forge         
nodejs                        12.4.0      he1b5a44_0  conda-forge         
nodejs                        12.7.0      h10a4023_1  conda-forge         
nodejs                        12.7.0      he1b5a44_0  conda-forge         
nodejs                        12.8.0      h10a4023_1  conda-forge         
nodejs                        12.8.1      h10a4023_0  conda-forge         
nodejs                        12.9.0      h10a4023_0  conda-forge         
nodejs                        12.9.1      h10a4023_0  conda-forge         
nodejs                        13.0.0      h10a4023_1  conda-forge         
nodejs                       13.10.1      hf5d1a2b_0  conda-forge         
nodejs                       13.12.0      hf5d1a2b_0  conda-forge         
nodejs                       13.13.0      hf5d1a2b_0  conda-forge         
nodejs                        13.6.0      h10a4023_0  conda-forge         
nodejs                        13.7.0      h10a4023_0  conda-forge         
nodejs                        13.8.0      h10a4023_0  conda-forge         
nodejs                        13.9.0      h10a4023_0  conda-forge         
nodejs                        13.9.0      hf5d1a2b_1  conda-forge         
nodejs                       14.10.0      h568c755_0  conda-forge         
nodejs                       14.10.1      h568c755_0  conda-forge         
nodejs                       14.11.0      h568c755_0  conda-forge         
nodejs                       14.12.0      h568c755_0  conda-forge         
nodejs                       14.13.0      h568c755_0  conda-forge         
nodejs                       14.13.1      h568c755_0  conda-forge         
nodejs                       14.14.0      h25f6087_2  conda-forge         
nodejs                       14.14.0      h568c755_0  conda-forge         
nodejs                       14.14.0      h914e61d_0  conda-forge         
nodejs                       14.14.0      h914e61d_1  conda-forge         
nodejs                       14.15.1      h25f6087_0  conda-forge         
nodejs                       14.15.4      h92b4a50_0  conda-forge         
nodejs                       14.15.4      h92b4a50_1  conda-forge         
nodejs                       14.17.1      h92b4a50_1  conda-forge         
nodejs                       14.17.4      h92b4a50_0  conda-forge         
nodejs                       14.17.4      h92b4a50_1  conda-forge         
nodejs                       14.18.3      h8ca31f7_2  conda-forge         
nodejs                       14.18.3      h92b4a50_1  conda-forge         
nodejs                       14.18.3      h96d913c_3  conda-forge         
nodejs                       14.19.0      h96d913c_0  conda-forge         
nodejs                       14.19.1      h96d913c_0  conda-forge         
nodejs                       14.19.2      h96d913c_0  conda-forge         
nodejs                       14.19.3      h96d913c_0  conda-forge         
nodejs                        14.2.0      h568c755_1  conda-forge         
nodejs                        14.2.0      hf5d1a2b_0  conda-forge         
nodejs                       14.20.0      h96d913c_0  conda-forge         
nodejs                       14.20.1      h96d913c_0  conda-forge         
nodejs                       14.20.1      h96d913c_1  conda-forge         
nodejs                       14.20.1      hb42c98f_1  conda-forge         
nodejs                        14.3.0      h568c755_0  conda-forge         
nodejs                        14.4.0      h568c755_0  conda-forge         
nodejs                        14.5.0      h568c755_0  conda-forge         
nodejs                        14.6.0      h568c755_0  conda-forge         
nodejs                        14.7.0      h568c755_0  conda-forge         
nodejs                        14.8.0      h568c755_0  conda-forge         
nodejs                        14.9.0      h568c755_0  conda-forge         
nodejs                        15.1.0      h914e61d_0  conda-forge         
nodejs                       15.10.0      h92b4a50_0  conda-forge         
nodejs                       15.10.0      h92b4a50_1  conda-forge         
nodejs                       15.11.0      h92b4a50_0  conda-forge         
nodejs                       15.12.0      h92b4a50_0  conda-forge         
nodejs                       15.13.0      h92b4a50_0  conda-forge         
nodejs                       15.14.0      h92b4a50_0  conda-forge         
nodejs                        15.2.0      h914e61d_0  conda-forge         
nodejs                        15.2.1      h25f6087_1  conda-forge         
nodejs                        15.2.1      h914e61d_0  conda-forge         
nodejs                        15.3.0      h25f6087_0  conda-forge         
nodejs                        15.3.0      h92b4a50_1  conda-forge         
nodejs                        15.8.0      h92b4a50_1  conda-forge         
nodejs                       16.10.0      h73e1104_1  conda-forge         
nodejs                       16.10.0      h92b4a50_0  conda-forge         
nodejs                       16.10.0      h92b4a50_1  conda-forge         
nodejs                       16.11.1      h73e1104_0  conda-forge         
nodejs                       16.11.1      h92b4a50_0  conda-forge         
nodejs                       16.12.0      h73e1104_0  conda-forge         
nodejs                       16.12.0      h8ca31f7_1  conda-forge         
nodejs                       16.12.0      h92b4a50_0  conda-forge         
nodejs                       16.12.0      hb5e7134_1  conda-forge         
nodejs                       16.13.0      h8ca31f7_0  conda-forge         
nodejs                       16.13.0      h8ca31f7_1  conda-forge         
nodejs                       16.13.0      h8ca31f7_2  conda-forge         
nodejs                       16.13.0      hb5e7134_0  conda-forge         
nodejs                       16.13.0      hb5e7134_1  conda-forge         
nodejs                       16.13.0      hb5e7134_2  conda-forge         
nodejs                       16.13.2      h8ca31f7_2  conda-forge         
nodejs                       16.13.2      hb5e7134_2  conda-forge         
nodejs                       16.14.2      h784f1fd_0  conda-forge         
nodejs                       16.14.2      h8839609_1  conda-forge         
nodejs                       16.14.2      h96d913c_1  conda-forge         
nodejs                       16.14.2      hfba9c51_0  conda-forge         
nodejs                       16.15.0      h8839609_0  conda-forge         
nodejs                       16.15.0      h96d913c_0  conda-forge         
nodejs                       16.15.1      h8839609_0  conda-forge         
nodejs                       16.15.1      h96d913c_0  conda-forge         
nodejs                       16.16.0      h8839609_0  conda-forge         
nodejs                       16.16.0      h96d913c_0  conda-forge         
nodejs                       16.17.0      h8839609_0  conda-forge         
nodejs                       16.17.0      h96d913c_0  conda-forge         
nodejs                       16.17.1      h8839609_0  conda-forge         
nodejs                       16.17.1      h96d913c_0  conda-forge         
nodejs                        16.4.1      h92b4a50_0  conda-forge         
nodejs                        16.5.0      h92b4a50_0  conda-forge         
nodejs                        16.6.0      h92b4a50_0  conda-forge         
nodejs                        16.6.1      h92b4a50_0  conda-forge         
nodejs                        16.9.0      h92b4a50_0  conda-forge         
nodejs                        17.1.0      h8ca31f7_1  conda-forge         
nodejs                        17.1.0      h8ca31f7_2  conda-forge         
nodejs                        17.1.0      hb5e7134_1  conda-forge         
nodejs                        17.1.0      hb5e7134_2  conda-forge         
nodejs                        17.4.0      h8ca31f7_0  conda-forge         
nodejs                        17.4.0      hb5e7134_0  conda-forge         
nodejs                        17.7.1      h784f1fd_0  conda-forge         
nodejs                        17.7.1      hfba9c51_0  conda-forge         
nodejs                        17.8.0      h784f1fd_0  conda-forge         
nodejs                        17.8.0      h8839609_1  conda-forge         
nodejs                        17.8.0      h96d913c_1  conda-forge         
nodejs                        17.8.0      hfba9c51_0  conda-forge         
nodejs                        17.9.0      h8839609_0  conda-forge         
nodejs                        17.9.0      h96d913c_0  conda-forge         
nodejs                       18.10.0      h8839609_0  conda-forge         
nodejs                       18.10.0      h96d913c_0  conda-forge         
nodejs                       18.11.0      h8839609_0  conda-forge         
nodejs                       18.11.0      h96d913c_0  conda-forge         
nodejs                       18.12.1      h8839609_0  conda-forge         
nodejs                       18.12.1      h96d913c_0  conda-forge         
nodejs                        18.3.0      h8839609_0  conda-forge         
nodejs                        18.3.0      h96d913c_0  conda-forge         
nodejs                        18.5.0      h8839609_0  conda-forge         
nodejs                        18.5.0      h96d913c_0  conda-forge         
nodejs                        18.6.0      h8839609_0  conda-forge         
nodejs                        18.6.0      h96d913c_0  conda-forge         
nodejs                        18.7.0      h8839609_0  conda-forge         
nodejs                        18.7.0      h96d913c_0  conda-forge         
nodejs                        18.8.0      h8839609_0  conda-forge         
nodejs                        18.8.0      h96d913c_0  conda-forge         
nodejs                        18.9.0      h8839609_0  conda-forge         
nodejs                        18.9.0      h96d913c_0  conda-forge         
nodejs                        18.9.1      h8839609_0  conda-forge         
nodejs                        18.9.1      h96d913c_0  conda-forge         
nodejs                         4.4.1               0  conda-forge         
nodejs                         4.4.7               0  conda-forge         
nodejs                         4.5.0               0  conda-forge         
nodejs                        6.10.2               0  conda-forge         
nodejs                        6.11.0               0  conda-forge         
nodejs                        6.12.0               0  conda-forge         
nodejs                        6.12.2               0  conda-forge         
nodejs                        6.13.1               0  conda-forge         
nodejs                         6.6.0               0  conda-forge         
nodejs                        8.10.0               0  conda-forge         
nodejs                         8.8.1               0  conda-forge         
nodejs                         8.9.1               0  conda-forge         
nodejs                         8.9.3               0  conda-forge         
nodejs                        9.11.0               0  conda-forge         
nodejs                        9.11.1               0  conda-forge         
nodejs                         9.5.0               0  conda-forge         
nodejs                         9.9.0               0  conda-forge         
nodejs                       16.18.0      h8d033a5_0  conda-forge         
nodejs                       16.18.1      h8d033a5_0  conda-forge         
nodejs                       16.19.0      h4abf6b9_1  conda-forge         
nodejs                       16.19.0      h8d033a5_0  conda-forge         
nodejs                       16.20.2      h1990674_2  conda-forge         
nodejs                       16.20.2      hc19f0b3_3  conda-forge         
nodejs                       16.20.2      hf52ce11_1  conda-forge         
nodejs                       18.13.0      h8d033a5_0  conda-forge         
nodejs                       18.14.0      h8d033a5_0  conda-forge         
nodejs                       18.14.1      h8d033a5_0  conda-forge         
nodejs                       18.14.2      h8d033a5_0  conda-forge         
nodejs                       18.15.0      h4abf6b9_1  conda-forge         
nodejs                       18.15.0      h8d033a5_0  conda-forge         
nodejs                       18.16.1      hf52ce11_0  conda-forge         
nodejs                       18.17.1      h1990674_1  conda-forge         
nodejs                       18.17.1      hf52ce11_0  conda-forge         
nodejs                       18.18.2      hb753e55_1  conda-forge         
nodejs                       18.19.0      hb753e55_0  conda-forge         
nodejs                       18.20.2      h84e79e0_0  conda-forge         
nodejs                       18.20.3      h6d9b948_0  conda-forge         
nodejs                       18.20.3      hc19f0b3_1  conda-forge         
nodejs                       18.20.4      hc55a1b2_1  conda-forge         
nodejs                       18.20.5      h6f0827f_0  conda-forge         
nodejs                       18.20.5      h7bbc488_1  conda-forge         
nodejs                       18.20.5      hc38da69_2  conda-forge         
nodejs                       18.20.5      hc38da69_3  conda-forge         
nodejs                       18.20.5      hcc90b0c_1  conda-forge         
nodejs                       18.20.5      hcc90b0c_2  conda-forge         
nodejs                       18.20.5      hcc90b0c_3  conda-forge         
nodejs                       18.20.5      hf3d5445_0  conda-forge         
nodejs                        20.1.0      hf52ce11_0  conda-forge         
nodejs                       20.12.2      hb753e55_0  conda-forge         
nodejs                       20.12.2      hc19f0b3_1  conda-forge         
nodejs                       20.17.0      hc55a1b2_0  conda-forge         
nodejs                       20.18.0      hc55a1b2_0  conda-forge         
nodejs                       20.18.1      hc55a1b2_0  conda-forge         
nodejs                       20.19.3      hc55a1b2_0  conda-forge         
nodejs                       20.19.4      hf7ee748_0  conda-forge         
nodejs                       20.19.5      hc8774ed_1  conda-forge         
nodejs                       20.19.5      hf7ee748_0  conda-forge         
nodejs                       20.19.6      hc8774ed_0  conda-forge         
nodejs                       20.19.6      hfe5e47f_1  conda-forge         
nodejs                       20.19.6      hfe5e47f_2  conda-forge         
nodejs                       20.20.0      hfe5e47f_0  conda-forge         
nodejs                       20.20.1      h44b80ee_0  conda-forge         
nodejs                       20.20.2      h44b80ee_0  conda-forge         
nodejs                       20.20.2      h44b80ee_1  conda-forge         
nodejs                       20.20.2      h4dda4d0_2  conda-forge         
nodejs                        20.5.1      hb753e55_1  conda-forge         
nodejs                        20.5.1      hf52ce11_0  conda-forge         
nodejs                        20.6.0      hb753e55_0  conda-forge         
nodejs                        20.6.1      hb753e55_0  conda-forge         
nodejs                        20.7.0      hb753e55_0  conda-forge         
nodejs                        20.8.0      hb753e55_0  conda-forge         
nodejs                        20.8.1      h1990674_0  conda-forge         
nodejs                        20.9.0      hb753e55_0  conda-forge         
nodejs                        20.9.0      hb753e55_1  conda-forge         
nodejs                       22.11.0      hf235a45_0  conda-forge         
nodejs                       22.12.0      hf235a45_0  conda-forge         
nodejs                       22.13.0      hf235a45_0  conda-forge         
nodejs                       22.17.0      h3883d8a_0  conda-forge         
nodejs                       22.17.0      heeeca48_1  conda-forge         
nodejs                       22.17.1      heeeca48_0  conda-forge         
nodejs                       22.19.0      h4a9c4b4_1  conda-forge         
nodejs                       22.19.0      heeeca48_0  conda-forge         
nodejs                       22.21.1      h273caaf_1  conda-forge         
nodejs                       22.21.1      h273caaf_2  conda-forge         
nodejs                       22.21.1      h4a9c4b4_0  conda-forge         
nodejs                       22.22.0      h273caaf_0  conda-forge         
nodejs                       22.22.2      h273caaf_0  conda-forge         
nodejs                       22.22.3      h273caaf_0  conda-forge         
nodejs                       22.22.3      h273caaf_1  conda-forge         
nodejs                       22.23.1      h273caaf_0  conda-forge         
nodejs                       22.23.2      h273caaf_0  conda-forge         
nodejs                       22.23.2      h645b291_1  conda-forge         
nodejs                        22.3.0      h6d9b948_0  conda-forge         
nodejs                        22.4.0      h6d9b948_0  conda-forge         
nodejs                        22.4.1      h6d9b948_0  conda-forge         
nodejs                        22.5.0      h6d9b948_0  conda-forge         
nodejs                        22.5.1      h6d9b948_0  conda-forge         
nodejs                        22.6.0      h6d9b948_0  conda-forge         
nodejs                        22.6.0      hc19f0b3_1  conda-forge         
nodejs                        22.7.0      hf235a45_0  conda-forge         
nodejs                        22.8.0      hf235a45_0  conda-forge         
nodejs                        22.9.0      hf235a45_0  conda-forge         
nodejs                       24.10.0      h36edbcc_2  conda-forge         
nodejs                       24.10.0      h8c692ec_1  conda-forge         
nodejs                       24.11.1      h36edbcc_0  conda-forge         
nodejs                       24.12.0      h36edbcc_0  conda-forge         
nodejs                       24.13.0      h36edbcc_0  conda-forge         
nodejs                       24.13.0      h3d65ac4_1  conda-forge         
nodejs                       24.13.0      h3d65ac4_2  conda-forge         
nodejs                       24.13.1      h3d65ac4_0  conda-forge         
nodejs                       24.14.0      h3d65ac4_0  conda-forge         
nodejs                       24.14.1      h3d65ac4_0  conda-forge         
nodejs                       24.16.0      h3d65ac4_0  conda-forge         
nodejs                       24.16.0      h3d65ac4_1  conda-forge         
nodejs                       24.18.0      h3d65ac4_0  conda-forge         
nodejs                       24.19.0      h3d65ac4_0  conda-forge         
nodejs                       24.19.0      h50021de_1  conda-forge         
nodejs                       24.21.0      h50021de_0  conda-forge         
nodejs                        24.3.0      h029053a_0  conda-forge         
nodejs                        24.3.0      heeeca48_1  conda-forge         
nodejs                        24.4.0      heeeca48_0  conda-forge         
nodejs                        24.4.0      heeeca48_1  conda-forge         
nodejs                        24.4.1      heeeca48_0  conda-forge         
nodejs                        24.8.0      heeeca48_0  conda-forge         
nodejs                        24.9.0      h8c692ec_1  conda-forge         
nodejs                        24.9.0      heeeca48_0  conda-forge         
nodejs                        25.2.1      h547ee38_0  conda-forge         
nodejs                        25.2.1      h728ac57_2  conda-forge         
nodejs                        25.2.1      he2c55a7_1  conda-forge         
nodejs                        25.2.1      he4ff34a_3  conda-forge         
nodejs                        25.6.0      he4ff34a_0  conda-forge         
nodejs                        25.6.1      he4ff34a_0  conda-forge         
nodejs                        25.7.0      he4ff34a_0  conda-forge         
nodejs                        25.8.2      he4ff34a_0  conda-forge         
nodejs                        25.9.0      he4ff34a_0  conda-forge         
nodejs                        26.0.0      he4ff34a_0  conda-forge         
nodejs                        26.1.0      he4ff34a_0  conda-forge         
nodejs                        26.2.0      he4ff34a_0  conda-forge         
nodejs                        26.3.0      he4ff34a_0  conda-forge         
nodejs                        26.3.1      hc039f44_1  conda-forge         
nodejs                        26.3.1      he4ff34a_0  conda-forge         
nodejs                        26.4.0      hc039f44_0  conda-forge         
nodejs                        26.5.0      hc039f44_0  conda-forge         
nodejs                        26.5.1      hc039f44_0  conda-forge         
nodejs                        26.6.0      ha5cdc1e_1  conda-forge         
nodejs                        26.6.0      hc039f44_0  conda-forge         
nodejs                        26.8.0      ha5cdc1e_1  conda-forge         
nodejs                        26.8.1      ha5cdc1e_1  conda-forge         
nodejs                        26.8.2      ha5cdc1e_0  conda-forge         
mkandes@hardtack:~$ conda install nodejs
Channels:
 - conda-forge
Platform: linux-64
Collecting package metadata (repodata.json): done
Solving environment: done

## Package Plan ##

  environment location: /home/mkandes/software/miniforge/26.3.2-1

  added / updated specs:
    - nodejs


The following packages will be downloaded:

    package                    |            build
    ---------------------------|-----------------
    conda-26.7.2               |  py313hd5f5364_2         1.4 MB  conda-forge
    libabseil-20260526.0       | cxx17_h0dc7533_2         1.4 MB  conda-forge
    libuv-1.52.1               |       h280c20c_1         410 KB  conda-forge
    nodejs-26.8.2              |       ha5cdc1e_0        19.3 MB  conda-forge
    ------------------------------------------------------------
                                           Total:        22.4 MB

The following NEW packages will be INSTALLED:

  libabseil          conda-forge/linux-64::libabseil-20260526.0-cxx17_h0dc7533_2 
  libbrotlicommon    conda-forge/linux-64::libbrotlicommon-1.2.0-hb03c661_1 
  libbrotlidec       conda-forge/linux-64::libbrotlidec-1.2.0-hb03c661_1 
  libbrotlienc       conda-forge/linux-64::libbrotlienc-1.2.0-hb03c661_1 
  libuv              conda-forge/linux-64::libuv-1.52.1-h280c20c_1 
  nodejs             conda-forge/linux-64::nodejs-26.8.2-ha5cdc1e_0 

The following packages will be UPDATED:

  conda                              26.7.0-py313hd5f5364_0 --> 26.7.2-py313hd5f5364_2 
  openssl                                  3.6.3-h35e630c_0 --> 3.6.4-h781a0a9_0 


Proceed ([y]/n)? y


Downloading and Extracting Packages:
                                                                                                                                                        
Preparing transaction: done                                                                                                                             
Verifying transaction: done                                                                                                                             
Executing transaction: done                                                                                                                             
mkandes@hardtack:~$ conda env list

# conda environments:
#
# * -> active
# + -> frozen
base                     /home/mkandes/software/miniforge/26.3.2-1
adix                     /home/mkandes/software/miniforge/26.3.2-1/envs/adix
arathon                  /home/mkandes/software/miniforge/26.3.2-1/envs/arathon
doccano                  /home/mkandes/software/miniforge/26.3.2-1/envs/doccano
docusaurus               /home/mkandes/software/miniforge/26.3.2-1/envs/docusaurus
hpcgpt-zendesk-clean-20260512     /home/mkandes/software/miniforge/26.3.2-1/envs/hpcgpt-zendesk-clean-20260512
hpcgpt-zendesk-explore-20260512     /home/mkandes/software/miniforge/26.3.2-1/envs/hpcgpt-zendesk-explore-20260512
icicle-tapis-20260522     /home/mkandes/software/miniforge/26.3.2-1/envs/icicle-tapis-20260522
label-studio             /home/mkandes/software/miniforge/26.3.2-1/envs/label-studio
openai-20260612          /home/mkandes/software/miniforge/26.3.2-1/envs/openai-20260612

mkandes@hardtack:~$ conda activate docusaurus
(docusaurus) mkandes@hardtack:~$ conda install nodejs
Channels:
 - conda-forge
Platform: linux-64
Collecting package metadata (repodata.json): done
Solving environment: done

## Package Plan ##

  environment location: /home/mkandes/software/miniforge/26.3.2-1/envs/docusaurus

  added / updated specs:
    - nodejs


The following packages will be downloaded:

    package                    |            build
    ---------------------------|-----------------
    libbrotlicommon-1.2.0      |       h39a168f_4          79 KB  conda-forge
    libbrotlidec-1.2.0         |       ha411449_4          34 KB  conda-forge
    libbrotlienc-1.2.0         |       h018ffa1_4         291 KB  conda-forge
    libgcc-16.2.0              |       ha9f2e26_5         1.0 MB  conda-forge
    libgomp-16.2.0             |       he0feb66_5         627 KB  conda-forge
    libstdcxx-16.2.0           |       h934c35e_5         6.3 MB  conda-forge
    ------------------------------------------------------------
                                           Total:         8.3 MB

The following NEW packages will be INSTALLED:

  _openmp_mutex      conda-forge/linux-64::_openmp_mutex-4.5-20_gnu 
  c-ares             conda-forge/linux-64::c-ares-1.34.8-hebe6cf0_2 
  ca-certificates    conda-forge/noarch::ca-certificates-2026.7.22-hbd8a1cb_0 
  icu                conda-forge/linux-64::icu-78.3-py310h44b86e0_2 
  libabseil          conda-forge/linux-64::libabseil-20260526.0-cxx17_h0dc7533_2 
  libbrotlicommon    conda-forge/linux-64::libbrotlicommon-1.2.0-h39a168f_4 
  libbrotlidec       conda-forge/linux-64::libbrotlidec-1.2.0-ha411449_4 
  libbrotlienc       conda-forge/linux-64::libbrotlienc-1.2.0-h018ffa1_4 
  libev              conda-forge/linux-64::libev-4.33-h280c20c_3 
  libgcc             conda-forge/linux-64::libgcc-16.2.0-ha9f2e26_5 
  libgomp            conda-forge/linux-64::libgomp-16.2.0-he0feb66_5 
  libnghttp2         conda-forge/linux-64::libnghttp2-1.68.1-h74cf4be_1 
  libsqlite          conda-forge/linux-64::libsqlite-3.53.4-h13e7031_1 
  libstdcxx          conda-forge/linux-64::libstdcxx-16.2.0-h934c35e_5 
  libuv              conda-forge/linux-64::libuv-1.52.1-h280c20c_1 
  libzlib            conda-forge/linux-64::libzlib-1.3.2-h25fd6f3_3 
  nodejs             conda-forge/linux-64::nodejs-26.8.2-ha5cdc1e_0 
  openssl            conda-forge/linux-64::openssl-3.6.4-h781a0a9_0 
  zstd               conda-forge/linux-64::zstd-1.5.7-hb78ec9c_7 


Proceed ([y]/n)? y


Downloading and Extracting Packages:
libstdcxx-16.2.0     | 6.3 MB    | ############################################################################################################# | 100% 
libgcc-16.2.0        | 1.0 MB    | ############################################################################################################# | 100% 
libgomp-16.2.0       | 627 KB    | #################################################################################################libbrotlicommon-1.2. | 79 KB     | ######################1                                                                          libstdcxx-16.2.0     | 6.3 MB    | ############################################################################################################# | 100%  | 627 KB    | ############################################################################################################# | 100%  | 79 KB     |                                                                                                               |   0% 
libbrotlidec-1.2.0   | 34 KB     | ############################################################################################################# | 100% 
libgcc-16.2.0        | 1.0 MB    | #################################################################################################libgcc-16.2.0        | 1.0 MB    | #################################################################################################                                                                                                                                                        
                                                                                                                                                         | 79 KB     | #################################################################################################                                                                                                                                                        
                                                                                                                                    Preparing transaction: done
Verifying transaction: done                                                                                                         Executing transaction: done
(docusaurus) mkandes@hardtack:~$
```

```
(docusaurus) mkandes@hardtack:~$ npx create-docusaurus@latest my-website classic
Need to install the following packages:
create-docusaurus@3.10.2
Ok to proceed? (y) y
✔ Which language do you want to use? › TypeScript
[INFO] Creating new Docusaurus project...
[INFO] Installing dependencies with npm...
[SUCCESS] Created my-website.
[INFO] Inside that directory, you can run several commands:

  `npm start`
    Starts the development server.

  `npm run build`
    Bundles your website into static files for production.

  `npm run serve`
    Serves the built website locally.

  `npm run deploy`
    Publishes the website to GitHub pages.

We recommend that you begin by typing:

  `cd my-website`
  `npm start`

Happy building awesome websites!

npm notice
npm notice New major version of npm available! 11.19.1 -> 12.1.0
npm notice Changelog: https://github.com/npm/cli/releases/tag/v12.1.0
npm notice To update run: npm install -g npm@12.1.0
npm notice
(docusaurus) mkandes@hardtack:~$
```

```
(docusaurus) mkandes@hardtack:~$ npm install -g npm@12.1.0

removed 8 packages, and changed 72 packages in 4s

15 packages are looking for funding
  run `npm fund` for details
(docusaurus) mkandes@hardtack:~$
```

```
(docusaurus) mkandes@hardtack:~$ npx docusaurus start
npm error could not determine executable to run
npm error A complete log of this run can be found in: /home/mkandes/.npm/_logs/2026-09-23T14_33_45_648Z-debug-0.log
(docusaurus) mkandes@hardtack:~$ cd my-website/
(docusaurus) mkandes@hardtack:~/my-website$ npx docusaurus start
npm notice run my-website@0.0.0 npx
npm notice run 'docusaurus' start
[INFO] Starting the development server...
[SUCCESS] Docusaurus website is running at: http://localhost:3000/
● Client ██████████████████████████████████████████████████ (100%) emitting after emit                                              client (Rspack 1.7.12) compiled successfully
```

# References
- https://docusaurus.io/
- https://docusaurus.io/docs
- https://docusaurus.io/docs/styling-layout#styling-your-site-with-infima
- https://docusaurus.community/knowledge/design/css/variables/
- https://infima.dev/
- https://github.com/webbertakken/awesome-docusaurus

- https://github.com/facebook/docusaurus/issues/2692

- https://accessibility.ucsd.edu/policies-standards/ucsd-accessibility-guidelines.html
- https://blink.ucsd.edu/technology/websites/training/managing/siteimprove/index.html
- https://www.w3.org/TR/WCAG22/

- https://www.reddit.com/r/opensource/comments/1ob5s61/what_are_the_best_tools_or_platforms_for_creating/
- https://starlight.astro.build/
- https://zensical.org/
- https://talkpython.fm/episodes/show/542/zensical-a-modern-static-site-generator
- https://news.ycombinator.com/item?id=45865189
- https://docsio.co/blog/zensical-vs-docusaurus
- https://docs.readthedocs.com/platform/stable/intro/doctools.html
- https://quarto.org/

- https://diataxis.fr/

- https://relay.dev/
