...
8. If the server starts up successfully, open a second `Terminal` window and create an SSH tunnel with local port forwarding to the compute node on Expanse where the `vncserver` is running.
    ...
    [mkandes@exp-9-55 ~]$
    ```
9. Now try to connect to the headless VNC server running on Expanse with the built-in VNC client on your Mac. In the Finder menu, choose `Go > Connect to Server`. Type the network address `vnc://127.0.0.1:5901` in the Server Address field. And then click `Connect`.
