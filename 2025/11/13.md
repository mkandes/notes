...
3. Load the `singularitypro` module and then `shell` into the `tigervnc` container.
   ...
   [mkandes@exp-9-55 ~]$ singularity shell --bind /expanse,/run,/scratch oras://ghcr.io/mkandes/tigervnc:latest-amd64
...
6. After your password is set, create the `xstartup` file and set its permissions to be executable.
   ...
   Singularity>
   ```
7. Now try and start the `vncserver`.
   ```
   ...
   Singularity>
   ```
8. If the server starts up successfully, open a second `Terminal` window and create an SSH tunnel with local port forwarding to the compute node on Expanse where the `vncserver` is running.
    ```
    ...
    [mkandes@exp-9-55 ~]$
    ```
9. Now try to connect to the headless VNC server running on Expanse with the built-in VNC client on your Mac. In the Finder menu, choose `Go > Connect to Server`. Type the network address `vnc://127.0.0.1:5901` in the Server Address field andthen click `Connect`.
