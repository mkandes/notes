...
The initial JupyterLab session will take several minutes to start up AFTER the batch job itself starts running. You can check the status (`ST`) of the job with the `squeue` command.
*Command*
```
squeue -u $USER
```
