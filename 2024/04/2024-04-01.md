...
For this experiment, we'll set the hostname to after the ARA site and resource the container is running on and then select lease created above from **Lease Name** drop-down list. You may leave the **CPU** and **Memory** fields blank as by default ARA will allocate to the container the total number of CPUs and memory available on the requested resource.
