...
Here, in this case, we want to create development versions of the latest existing production Spack instance(s) on Expanse, namely, `expanse/0.17.3/cpu/b` and `expanse/0.17.3/gpu/b`. These instance(s) will allow us to more quickly test system changes and their possible affects on the already depolyed production Spack instances. In addition, these will serve as a the base of all new Spack instances when testing new, updated versions of Spack. See https://github.com/sdsc/spack/issues/124. As such, we'll create these new subset of instances as special versions of the named `cpu` and `gpu` production instances. These Spack instances will be given the `SPACK_INSTANCE_VERSION='dev'`. 
