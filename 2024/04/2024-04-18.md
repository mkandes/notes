...
Now that we have the configruation files ready to begin developing the `cpu/dev` instance, let's try and plan out an initial deployment based on the output from a dump of the production `cpu` instances current environment.
